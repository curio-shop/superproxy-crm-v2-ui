...
export default function NewEmail({ onBack, contactName = 'Pete Salvador', contactEmail = 'pete@example.com', onOpenEmailHistory, preSelectedQuote, preSelectedInvoice }: NewEmailProps) {
  ...
  const [emailBody, setEmailBody] = useState(`Hi ${contactName.split(' ')[0]},
      ...
      <div className="flex-1 overflow-y-auto px-4 lg:px-8 pb-8 flex justify-center custom-scrollbar bg-slate-50/30">
        <div className="w-full max-w-5xl flex flex-col gap-6 pt-6">
          ...
          <div className="flex flex-col bg-white border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl ring-1 ring-slate-900/5 transition-all duration-300 hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.06)] relative z-10">
            ...
            {/* Modern Attachment Footer */}
            <div className="px-8 py-5 border-t border-slate-50 bg-[#FBFBFC]">
              <div className="inline-flex items-center gap-3 py-2.5 pl-2 pr-4 transition-all bg-white border cursor-pointer border-slate-200/60 rounded-xl shadow-sm hover:shadow-md hover:ring-2 group hover:border-purple-200 hover:ring-purple-50">
                <div className="flex items-center justify-center w-11 h-11 transition-colors border rounded-lg bg-red-50 border-red-100 text-red-500 group-hover:scale-105 duration-200">
                ...
                <div className="relative group/tooltip">
                  <button className="p-1.5 ml-2 transition-all opacity-0 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 group-hover:opacity-100">
                    <Icon icon="solar:trash-bin-trash-linear" width="16" />
                  </button>
                  <span className="absolute left-1/2 -translate-x-1/2 top-full mt-2 bg-slate-900 text-white text-xs font-medium px-2.5 py-1.5 rounded-lg shadow-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                    Remove
                  </span>
                </div>
              </div>
